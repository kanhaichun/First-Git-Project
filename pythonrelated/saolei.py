...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 15 13:40:40 2017
...
if not ctrlData.hWnd:
    win32api.MessageBox(0, "please run windows minecrack 请先运行扫雷程序", " error 错误！", win32con.MB_ICONERROR)
    exit(0)
...
# 读取雷区数据
bytesRead = c_ulong(0)
ReadProcessMemory(hProc, BOARD_ADDR, byref(
    ctrlData.board), SMineCtrl.board.size, byref(bytesRead))
if(bytesRead.value != SMineCtrl.board.size):
    str = "ReadProcessMemory error, only read %d" % bytesRead.value + \
        " should read %d" % SMineCtrl.board.size
