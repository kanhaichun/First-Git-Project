...
s.display()
fv=["apple","banana","watermelon","pear","grape"]
state="initiate"
while state!="E":
    fruit_wanted=input("fruit you want:")
    while fruit_wanted not in fv:#loop until the fruit chosen is available
        fruit_wanted=input("Fruit not available! fruit You want: ")
    ...
    while fruitquantity_wanted<=0:#loop until the quantity is a positive integer
        fruitquantity_wanted=int(input("Not a positive integer! quantity you want: "))
    s.update(fruit_wanted,fruitquantity_wanted)
    print("input C to buy fruit or to continue")
    print("input E to end")#choose to continue or end
